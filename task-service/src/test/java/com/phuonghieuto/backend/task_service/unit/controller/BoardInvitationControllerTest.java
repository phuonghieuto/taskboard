...
class BoardInvitationControllerTest {
        ...
        void setUp() {
                ...
                invitationResponse = BoardInvitationResponseDTO.builder().id(TEST_INVITATION_ID).boardId(TEST_BOARD_ID)
                                ...
                                .inviteeUserId(TEST_USER_ID).token(TEST_TOKEN).status(InvitationStatus.PENDING)
                                .createdAt(now).expiresAt(now.plusHours(48)).build();
                ...
                BoardInvitationResponseDTO invitation2 = BoardInvitationResponseDTO.builder().id("invitation-id-2")
                                ...
                                .inviteeEmail("another@example.com").inviteeUserId("another-user-id")
                                .token("another-token").status(InvitationStatus.PENDING).createdAt(now)
        ...
        void createInvitation_Success() throws Exception {
                ...
                mockMvc.perform(post("/board-invitations/board/{boardId}", TEST_BOARD_ID)
                                ...
                                .content(objectMapper.writeValueAsString(invitationRequest))).andExpect(status().isOk())
        ...
        void createInvitation_InvalidRequest() throws Exception {
                ...
                verify(boardInvitationService, times(0)).createInvitation(anyString(),
                                any(BoardInvitationRequestDTO.class));
        }
        ...
        void createInvitation_DuplicateInvitation() throws Exception {
                when(boardInvitationService.createInvitation(eq(TEST_BOARD_ID), any(BoardInvitationRequestDTO.class)))
                                .thenThrow(new DuplicateInvitationException(
                ...
                mockMvc.perform(post("/board-invitations/board/{boardId}", TEST_BOARD_ID)
                                ...
                                .andExpect(jsonPath("$.message")
                                                .value("A pending invitation already exists for this email"));
        ...
        void createInvitation_Unauthorized() throws Exception {
                ...
                mockMvc.perform(post("/board-invitations/board/{boardId}", TEST_BOARD_ID)
                                ...
                                .andExpect(jsonPath("$.message")
                                                .value("You don't have permission to invite users to this board"));
        ...
        void getInvitation_NotFound() throws Exception {
                when(boardInvitationService.getInvitationById(TEST_INVITATION_ID)).thenThrow(
                                new InvitationNotFoundException("Invitation not found with ID: " + TEST_INVITATION_ID));

                mockMvc.perform(get("/board-invitations/{id}", TEST_INVITATION_ID)).andExpect(status().isNotFound())
                                .andExpect(jsonPath("$.isSuccess").value(false)).andExpect(jsonPath("$.message")
                                                .value("Invitation not found with ID: " + TEST_INVITATION_ID));
        ...
        void getInvitation_Unauthorized() throws Exception {
                when(boardInvitationService.getInvitationById(TEST_INVITATION_ID)).thenThrow(
                                new UnauthorizedAccessException("You don't have permission to view this invitation"));

                mockMvc.perform(get("/board-invitations/{id}", TEST_INVITATION_ID)).andExpect(status().isUnauthorized())
                                .andExpect(jsonPath("$.isSuccess").value(false)).andExpect(jsonPath("$.message")
                                                .value("You don't have permission to view this invitation"));
        ...
        void getPendingInvitationsForBoard_Success() throws Exception {
                when(boardInvitationService.getPendingInvitationsForBoard(TEST_BOARD_ID))
                                .thenReturn(invitationResponseList);
        ...
        void getPendingInvitationsForBoard_BoardNotFound() throws Exception {
                ...
                mockMvc.perform(get("/board-invitations/board/{boardId}", TEST_BOARD_ID))
                                .andExpect(status().isNotFound()).andExpect(jsonPath("$.isSuccess").value(false))
                                .andExpect(jsonPath("$.message").value("Board not found with ID: " + TEST_BOARD_ID));
        ...
        void getMyPendingInvitations_Success() throws Exception {
                ...
                when(jwt.getClaim("userEmail")).thenReturn(TEST_EMAIL);
                when(boardInvitationService.getPendingInvitationsForUser(TEST_EMAIL))
                                .thenReturn(invitationResponseList);

                mockMvc.perform(get("/board-invitations/my-invitations").principal(authentication))
        ...
        void acceptInvitation_Success() throws Exception {
                BoardInvitationResponseDTO acceptedInvitation = BoardInvitationResponseDTO.builder()
                                .id(TEST_INVITATION_ID).boardId(TEST_BOARD_ID).boardName(TEST_BOARD_NAME)
                                .inviterUserId(TEST_INVITER_ID).inviteeEmail(TEST_EMAIL).inviteeUserId(TEST_USER_ID)
                                .token(TEST_TOKEN).status(InvitationStatus.ACCEPTED).createdAt(LocalDateTime.now())
                ...
                verify(boardInvitationService, times(1)).updateInvitationStatus(TEST_INVITATION_ID,
        ...
        void acceptInvitation_NotFound() throws Exception {
                when(boardInvitationService.updateInvitationStatus(TEST_INVITATION_ID, InvitationStatus.ACCEPTED))
                                .thenThrow(new InvitationNotFoundException(
                                                "Invitation not found with ID: " + TEST_INVITATION_ID));

                mockMvc.perform(put("/board-invitations/{id}/accept", TEST_INVITATION_ID))
                                .andExpect(status().isNotFound()).andExpect(jsonPath("$.isSuccess").value(false))
                                .andExpect(jsonPath("$.message")
                                                .value("Invitation not found with ID: " + TEST_INVITATION_ID));
        ...
        void acceptInvitation_Unauthorized() throws Exception {
                when(boardInvitationService.updateInvitationStatus(TEST_INVITATION_ID, InvitationStatus.ACCEPTED))
                                .thenThrow(new UnauthorizedAccessException(
                                                "You don't have permission to update this invitation"));

                mockMvc.perform(put("/board-invitations/{id}/accept", TEST_INVITATION_ID))
                                .andExpect(status().isUnauthorized()).andExpect(jsonPath("$.isSuccess").value(false))
                                .andExpect(jsonPath("$.message")
                                                .value("You don't have permission to update this invitation"));
        ...
        void declineInvitation_Success() throws Exception {
                BoardInvitationResponseDTO declinedInvitation = BoardInvitationResponseDTO.builder()
                                .id(TEST_INVITATION_ID).boardId(TEST_BOARD_ID).boardName(TEST_BOARD_NAME)
                                .inviterUserId(TEST_INVITER_ID).inviteeEmail(TEST_EMAIL).inviteeUserId(TEST_USER_ID)
                                .token(TEST_TOKEN).status(InvitationStatus.DECLINED).createdAt(LocalDateTime.now())
                                .expiresAt(LocalDateTime.now().plusHours(48)).build();
        ...
        void declineInvitation_NotFound() throws Exception {
                ...
                mockMvc.perform(put("/board-invitations/{id}/decline", TEST_INVITATION_ID))
                                .andExpect(status().isNotFound()).andExpect(jsonPath("$.isSuccess").value(false))
                                .andExpect(jsonPath("$.message")
                                                .value("Invitation not found with ID: " + TEST_INVITATION_ID));
        ...
        void cancelInvitation_Unauthorized() throws Exception {
                ...
                mockMvc.perform(delete("/board-invitations/{id}", TEST_INVITATION_ID))
                                .andExpect(status().isUnauthorized()).andExpect(jsonPath("$.isSuccess").value(false))
                                .andExpect(jsonPath("$.message")
                                                .value("Only the inviter or board owner can cancel this invitation"));
